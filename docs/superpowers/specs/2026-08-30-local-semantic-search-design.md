...
- **Model asset:** `universal_sentence_encoder.tflite` (~6 MB, 100-dim output) committed to `apps/mobile/android/app/src/main/assets/`. Treated as a release artifact like `apps/mobile/src/data/playCategories.json`. Adds ~6 MB to APK size.
...
4. Confirm APK size increase is ~6 MB and the app launches without an ANR (model loads off the main thread or lazily).
